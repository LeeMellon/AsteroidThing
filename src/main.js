...
$(document).ready(function() {
  // exampleStuff()
  // let showExample = example.init();
  // $('#solarsystem').append(showExample);
  // let solarsystem = document.getElementById('#solarsystem');
  // solarsytem.appendChild(SolarSystem.animate(), SolarSystem.domElement);

//solarsystem 3d

var ctx = document.getElementById('solar').appendChild(document.createElement('canvas')).getContext('2d'),
...
//end of solarsystem 3d
  ...
  promise.then(function(response) {
    ...
    for(let i = 0; i < Object.keys(asteroid.near_earth_objects).length; i++){
      ...
      console.log(asteroid.near_earth_objects[thisAsteroid].length);
      for(let j = 0; j < asteroid.near_earth_objects[thisAsteroid].length; j++){
        ...
        console.log(asteroid.near_earth_objects[thisAsteroid][j].close_approach_data[0].miss_distance.miles);
        if (threat == true){
          $('#roids').append("<li>" + `name:  ${asteroid.near_earth_objects[thisAsteroid][j].name}` + "</li>")
          let distance = asteroid.near_earth_objects[thisAsteroid][j].close_approach_data[0].miss_distance.miles;
          $('#threatDistance').append("<li>" + `${asteroid.near_earth_objects[thisAsteroid][j].close_approach_data[0].miss_distance.miles}` + " miles" + "</li>")
          ...
          $('#velocity').append("<li>" + `${velocityF}` + " mph" + "</li>")
          let timeUntilImpact = Math.floor((distance/velocity)/24);
          $('#timeUntilImpact').append("<li>" + timeUntilImpact + " days" + "</li>")
